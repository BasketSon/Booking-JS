...
(function () {
  ...
  noticeForm.addEventListener('reset', function () {
    ...
    document.querySelectorAll('.loaded-pin').forEach(function (it) {
      it.classList.add('visuallyhidden');
    });
  ...
  var successHandler = function () {
    ...
    window.map.resetPin();
    window.clearFormImages();
    window.map.mapPin.getPinAddress();
    var node = document.createElement('div');
    node.style = 'z-index: 100; margin: 0 auto; text-align: center; color: #fff; background-color: yellowgreen; border-radius: 20px;padding: 30px;';
    ...
    node.textContent = 'Успешная отправка формы. Нажми ESC, чтобы закрыть сообщение';
